...
class _DropSelectMoreState extends State<DropSelectMore> {
  ...
  Future<void> _scoreSortList() async {
    ...
    if (widget.mo.url.contains('_rsp')) {
      List<FromTemplateList>? res = await CoService.fire<List<FromTemplateList>>(widget.mo.url, params: map, key: widget.mo.url.urlQuery()['_rsp'] ?? '');
      if (res != null) {
        ls = res;
        if (mounted) setState(() {});
      }
    } else {
      FromTemplate? res = await CoService.fire<FromTemplate>(widget.mo.url, params: map);
      if (res != null) {
        ls = res.list;
        if (mounted) setState(() {});
      }
